...
@Component({
  selector: 'app-web-config-view-edit',
  templateUrl: './web-config-view-edit.component.html',
  styleUrls: ['./web-config-view-edit.component.scss']
})
export class WebConfigViewEditComponent implements OnInit, OnDestroy {
  isSaving = false;
  loading = false;
  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ webConfig }) => {
      ...
      this.updateForm(webConfig);
    });
  }

  updateForm(webConfig: IWebConfig): void {
    this.editForm.patchValue({
      id: webConfig.id,
      activeFlag: webConfig.activeFlag,
      webThemeId: webConfig.webThemeId,
    });
  }
