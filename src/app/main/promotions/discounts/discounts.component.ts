...
export class DiscountsComponent implements OnInit {
  ...
  loadPage(page?: number): void {
    ...
    this.discountsService
      ...
      .subscribe(
        (res: HttpResponse<IDiscounts[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IDiscounts): number {
    return item.id!;
  }

  registerChangeInDiscounts(): void {
    this.eventSubscriber = this.eventManager.subscribe('discountsModification', () => this.loadPage());
  }

  deleteConfirm(discounts: IDiscounts): void {
    this.confirmModal = this.modal.confirm({
      ...
      nzOnOk: () =>
        this.discountsService.delete(discounts.id).subscribe(() => {
          this.eventManager.broadcast('discountsModification');
        })
    });
  }
  ...
  protected onSuccess(data: IDiscounts[] | null, headers: HttpHeaders, page: number): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.router.navigate(['/main/promotions/discounts'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
